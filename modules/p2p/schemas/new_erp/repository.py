"""New ERP P2P Repository — implements P2PRepositoryProtocol for New ERP schema."""
...
class NewERPRepository:
    """New ERP P2P data access repository."""
    ...
    def query_purchase_orders(
        ...
        with self._session_factory() as session:
            stmt = (
                select(
                    NewPoHeader.po_number,
                    NewPoHeader.vendor_id,
                    NewPoHeader.vendor_name,
                    NewPoLine.category_id.label("material_category"),
                    NewPoHeader.total_amount.label("po_amount"),
                    NewPoLine.quantity.label("po_quantity"),
                    NewPoLine.unit_price,
                    NewPoLine.standard_price.label("contract_price"),
                    NewPoHeader.status,
                    NewPoHeader.creation_date,
                    NewPoLineLocation.promised_date.label("required_date"),
                    NewPoLine.item_id.label("material_code"),
                    NewPoLine.item_description.label("material_name"),
                    NewPoLine.line_num.label("line_number"),
                )
                .join(NewPoLine, NewPoHeader.po_header_id == NewPoLine.po_header_id)
                .join(NewPoLineLocation, NewPoLine.po_line_id == NewPoLineLocation.po_line_id)
            )

            if vendor_id:
                stmt = stmt.where(NewPoHeader.vendor_id == vendor_id)
            if status:
                stmt = stmt.where(NewPoHeader.status == status.upper())
            if po_number:
                stmt = stmt.where(NewPoHeader.po_number == po_number)
            if days > 0:
                stmt = stmt.where(
                    NewPoHeader.creation_date >= date.today() - timedelta(days=days)
                )

            stmt = _apply_order_and_limit(
                stmt, order_by, limit,
                date_col=NewPoHeader.creation_date,
                amount_col=NewPoHeader.total_amount,
            )
    ...
    def query_receipts(
        ...
        with self._session_factory() as session:
            stmt = select(NewRcvTransaction)

            if po_number:
                stmt = stmt.where(NewRcvTransaction.po_number == po_number)
            if vendor_id:
                stmt = stmt.where(NewRcvTransaction.vendor_id == vendor_id)
            if days > 0:
                stmt = stmt.where(
                    NewRcvTransaction.transaction_date >= date.today() - timedelta(days=days)
                )

            stmt = _apply_order_and_limit(
                stmt, order_by, limit,
                date_col=NewRcvTransaction.transaction_date,
                amount_col=None,
            )
    ...
    def query_invoices(
        ...
        with self._session_factory() as session:
            stmt = select(NewApInvoice)

            if invoice_num:
                stmt = stmt.where(NewApInvoice.invoice_num == invoice_num)
            if po_number:
                stmt = stmt.where(NewApInvoice.po_number == po_number)
            if vendor_id:
                stmt = stmt.where(NewApInvoice.vendor_id == vendor_id)
            if status:
                stmt = stmt.where(NewApInvoice.approval_status == status.upper())
            if days > 0:
                stmt = stmt.where(
                    NewApInvoice.invoice_date >= date.today() - timedelta(days=days)
                )

            stmt = _apply_order_and_limit(
                stmt, order_by, limit,
                date_col=NewApInvoice.invoice_date,
                amount_col=NewApInvoice.invoice_amount,
            )
    ...
    def query_payments(
        ...
        with self._session_factory() as session:
            stmt = select(NewApPayment)

            if check_number:
                stmt = stmt.where(NewApPayment.check_number == check_number)
            if invoice_num:
                stmt = stmt.where(NewApPayment.invoice_num == invoice_num)
            if vendor_id:
                stmt = stmt.where(NewApPayment.vendor_id == vendor_id)
            if days > 0:
                stmt = stmt.where(
                    NewApPayment.check_date >= date.today() - timedelta(days=days)
                )

            stmt = _apply_order_and_limit(
                stmt, order_by, limit,
                date_col=NewApPayment.check_date,
                amount_col=NewApPayment.amount,
            )
    ...
    def get_flattened_payments(
        ...
        with self._session_factory() as session:
            stmt = (
                select(NewApPayment)
                .join(NewApInvoice, NewApPayment.invoice_num == NewApInvoice.invoice_num)
                .where(NewApInvoice.po_number == po_number)
            )
            if vendor_id:
                stmt = stmt.where(NewApPayment.vendor_id == vendor_id)
            rows = session.scalars(stmt).all()
    ...
    def query_suppliers(
        ...
        with self._session_factory() as session:
            stmt = select(NewApSupplier)
            if vendor_id:
                stmt = stmt.where(NewApSupplier.vendor_id == vendor_id)
            rows = session.scalars(stmt).all()
            return [
                {
                    "vendor_id": r.vendor_id,
        ...
        with self._contract_price_lock:
            ...
            with self._session_factory() as session:
                rows = session.execute(
                    select(NewPoLine.item_id, NewPoLine.standard_price).distinct()
                ).all()
    ...
    def analyze_receipt_anomalies(
        ...
        with self._session_factory() as session:
            ...
            if po_number:
                stmt = stmt.where(NewRcvTransaction.po_number == po_number)
            stmt = stmt.where(NewRcvTransaction.transaction_date >= cutoff)
            receipts = session.scalars(stmt).all()

            for rcv in receipts:
                po_line = session.get(NewPoLine, rcv.po_line_id)
                if po_line is None:
                    continue
    ...
    def detect_duplicate_invoices(
        ...
        with self._session_factory() as session:
            stmt = select(NewApInvoice).where(NewApInvoice.invoice_date >= cutoff)
            if vendor_id:
                stmt = stmt.where(NewApInvoice.vendor_id == vendor_id)
    ...
    def analyze_discount_utilization(
        ...
        with self._session_factory() as session:
            stmt = (
                select(NewApInvoice)
                .where(
                    NewApInvoice.invoice_date >= cutoff,
                    NewApInvoice.discount_due_date.isnot(None),
                )
            )
            if vendor_id:
                stmt = stmt.where(NewApInvoice.vendor_id == vendor_id)
            ...
            for inv in invoices:
                payment = session.execute(
                    select(NewApPayment)
                    .where(NewApPayment.invoice_num == inv.invoice_num)
                    .limit(1)
                ).scalar()
        ...
        with self._session_factory() as session:
            stmt = (
                select(
                    NewPoHeader.vendor_id,
                    NewPoHeader.vendor_name,
                    func.sum(NewPoHeader.total_amount).label("total_spend"),
                    func.count(func.distinct(NewPoHeader.po_number)).label("order_count"),
                )
                .where(NewPoHeader.creation_date >= cutoff)
                .group_by(NewPoHeader.vendor_id, NewPoHeader.vendor_name)
                .order_by(func.sum(NewPoHeader.total_amount).desc())
            )
            ...
            cat_stmt = (
                select(
                    NewPoLine.category_id,
                    func.count(func.distinct(NewPoHeader.vendor_id)).label("vendor_count"),
                )
                .join(NewPoHeader, NewPoHeader.po_header_id == NewPoLine.po_header_id)
                .where(NewPoHeader.creation_date >= cutoff)
                .group_by(NewPoLine.category_id)
                .having(func.count(func.distinct(NewPoHeader.vendor_id)) == 1)
            )
    ...
    def calculate_po_cycle_time(
        ...
        with self._session_factory() as session:
            stmt = select(NewPoHeader).where(NewPoHeader.creation_date >= cutoff)
            if vendor_id:
                stmt = stmt.where(NewPoHeader.vendor_id == vendor_id)
            ...
            for po in orders:
                ...
                first_receipt = session.execute(
                    select(func.min(NewRcvTransaction.transaction_date))
                    .where(NewRcvTransaction.po_number == po.po_number)
                ).scalar()
                ...
                if first_receipt:
                    ...
                    first_payment_date = session.execute(
                        select(func.min(NewApPayment.check_date))
                        .join(NewApInvoice, NewApPayment.invoice_num == NewApInvoice.invoice_num)
                        .where(NewApInvoice.po_number == po.po_number)
                    ).scalar()
