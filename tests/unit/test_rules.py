...
class TestThreeWayMatchChecker:
    ...
    def test_no_anomaly(self, checker: ThreeWayMatchChecker) -> None:
        ...
        inv = [{"po_number": "PO-001", "invoice_num": "INV-001", "invoice_amount": 100000.0}]
    ...
    def test_amount_anomaly(
        ...
        # INV-001 偏差 12%，应被检测
        inv001 = [a for a in amount_anomalies if a.documents.invoice_num == "INV-001"]
        assert len(inv001) == 1
    ...
    def test_quantity_anomaly(
        ...
        inv = [{"po_number": p["po_number"], "invoice_num": f"INV-{i}", "invoice_amount": p["po_amount"]}
               for i, p in enumerate(mock_po_data, 1)]
    ...
    def test_severity_high_by_amount(self, checker: ThreeWayMatchChecker) -> None:
        ...
        inv = [{"po_number": "PO-X", "invoice_num": "INV-X", "invoice_amount": 640000.0}]
    ...
    def test_severity_medium(self, checker: ThreeWayMatchChecker) -> None:
        ...
        # 偏差 7%（容差 5% 的 1.4 倍）
        inv = [{"po_number": "PO-M", "invoice_num": "INV-M", "invoice_amount": 10700.0}]
    ...
    def test_severity_low_boundary(self, checker: ThreeWayMatchChecker) -> None:
        ...
        inv = [{"po_number": "PO-L", "invoice_num": "INV-L", "invoice_amount": 10460.0}]
    ...
    def test_supplier_tolerance(self, p2p_settings: P2PSettings) -> None:
        ...
        inv = [{"po_number": "PO-S", "invoice_num": "INV-S", "invoice_amount": 11200.0}]
...
class TestPriceVarianceAnalyzer:
    ...
    def test_price_variance_detected(self, analyzer: PriceVarianceAnalyzer) -> None:
        ...
        lines = [{"po_number": "PO-001", "material_code": "MAT-001", "unit_price": 230.0,
                  "line_number": "1", "vendor_name": "供应商A", "material_name": "轴承"}]
        prices = {"MAT-001": 200.0}
        result = analyzer.analyze(lines, prices)
        assert len(result) == 1
...
class TestPaymentComplianceChecker:
    ...
    def test_payment_overdue(self, checker: PaymentComplianceChecker) -> None:
        """逾期付款应被检测。"""
        payments = [{"check_number": "PAY-001", "invoice_num": "INV-001",
                     "check_date": "2026-04-11", "amount": 112000.0}]
        invoices = [{"invoice_num": "INV-001", "po_number": "PO-001",
                     "due_date": "2026-04-01", "invoice_amount": 112000.0, "vendor_name": "A"}]
    ...
    def test_payment_early(self, checker: PaymentComplianceChecker) -> None:
        """提前付款超过阈值应被检测。"""
        payments = [{"check_number": "PAY-002", "invoice_num": "INV-002",
                     "check_date": "2026-03-20", "amount": 50000.0}]
        invoices = [{"invoice_num": "INV-002", "po_number": "PO-002",
                     "due_date": "2026-04-05", "invoice_amount": 50000.0, "vendor_name": "A"}]
        result = checker.check(payments, invoices)
    ...
    def test_payment_no_anomaly(self, checker: PaymentComplianceChecker) -> None:
        """正常付款不应产生异常。"""
        payments = [{"check_number": "PAY-003", "invoice_num": "INV-003",
                     "check_date": "2026-04-03", "amount": 50000.0}]
        invoices = [{"invoice_num": "INV-003", "po_number": "PO-003",
                     "due_date": "2026-04-05", "invoice_amount": 50000.0, "vendor_name": "B"}]
        result = checker.check(payments, invoices)
        assert len(result) == 0

    def test_discount_abuse(self, checker: PaymentComplianceChecker) -> None:
        """折扣滥用应被检测（超过折扣截止日仍按折扣付款）。"""
        payments = [{"check_number": "PAY-004", "invoice_num": "INV-004",
                     "check_date": "2026-04-20", "amount": 76000.0}]
        invoices = [{"invoice_num": "INV-004", "po_number": "PO-004",
                     "due_date": "2026-04-15", "discount_due_date": "2026-04-05",
                     "invoice_amount": 80000.0, "vendor_name": "B"}]
        result = checker.check(payments, invoices)
...
class TestSupplierPerformanceCalculator:
    ...
    def test_kpi_good(self, calc: SupplierPerformanceCalculator) -> None:
        ...
        assert report.vendor_id == "SUP-001"
